...
def trigger_n8n_webhook(payload):
    """
    Triggers a webhook on n8n.io with the provided payload.

    Args:
        payload (dict): The data payload to be sent to the webhook.

    Prints:
        - If successful: "n8n webhook triggered successfully."
        - If failed: "Failed to trigger webhook. Status code: {response status code}"
        - If an error occurs during the process: "Error triggering n8n webhook: {error message}"

    This function sends a POST request to the n8n webhook URL with the provided payload.
    ...
    """
    # Construct the payload to be sent to the n8n webhook
    n8n_payload = {
        'payload': payload
    }

    try:
        # Send a POST request to the n8n webhook URL with the payload
...
def webhook():
    """
    Webhook endpoint for N8N to handle email response data.

    This endpoint expects a JSON payload containing the email response data.
    The data is extracted from the payload and passed to the `parse_email_response`
...
def handle_trigger():
    ...
    This function executes a SQL query and checks if the result is empty. If the result is empty, it sends an email
    request using Gmail and triggers a webhook indicating that an email has been received. It then generates code based on a
    description and prints it. If the result is not empty, it prints a message. If there is a latest response email ID,
    it parses the email data, updates the SQL table, and triggers webhooks indicating that the email has been parsed and
    the automation workflow has completed.
    ...
    try:
        ...
        if email_data:
            ...
            trigger_n8n_webhook(zapier_payload)
